...
#         self.ckp.save_in_training(self, epoch, is_best=False)
    ...
    def test_bench(self):
        ...
        self.model.eval()
        ssim_log = torch.zeros(1, len(self.loader_test), 3)
        ...
        for idx_data, d in enumerate(self.loader_test):
            for lr, hr, filename in tqdm(d, ncols=80):
                ...
                psnr_list[-1, idx_data, self.args.scale-2] += utility.calc_psnr(
                    sr, hr, self.args.scale, self.args.rgb_range, if_benchmark=True)
                ssim_log[-1, idx_data, self.args.scale-2] += utility.calc_SSIM(
            ...
            ssim_log[-1, idx_data, self.args.scale-2] /=len(d)
            print(
                '[{} x{}] \t PSNR: {:.3f}) \t SSIM: {:.4f}) \n'.format(
                    d.dataset.name,
                    self.args.scale,
                    psnr_list[-1, idx_data, self.args.scale-2].numpy(),
                    ssim_log[-1, idx_data, self.args.scale-2].numpy()
                ))
        torch.set_grad_enabled(True)
